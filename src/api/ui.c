...
void ui_disp_adj_xfixed_static_3(uint8_t* str,uint16_t x,uint8_t dp)
{
    uint8_t buf[10];
	ui_disp_clear_num_dp();
	if(x>999)x=999;
...
void ui_disp_menu_pdc(void)
{
	...
	x=stSysData.producteDate;
	if(x>9999)x=9999;
	m_int16_2_str_4(buf,x);
	
	x=stSysData.producteCode;
	if(x>9999)x=9999;
	m_int16_2_str_4(buf+4,x);
...
void ui_disp_menu_set_pdc(void)
{
    uint8_t buf[10];
    uint16_t x0,x1;
	uint8_t loc;
	...
	x0=stSysData.producteDate;
	x1=stSysData.producteCode;
	
	if(subMenu ==sub_MENU_SET_PD){
		x0=*((uint16_t*)(&adjValue));
	}else{
		x1=*((uint16_t*)(&adjValue));
	}
	
	m_int16_2_str_4(buf,x0);
	m_int16_2_str_4(buf+4,x1);
	...
	if(!fi_lcd_twinkle_lock()){
		if(!fi_twinkle())buf[subMenu*4+loc]=' ';
	}	
...
void ui_disp_menu_home(void)
{
	...
	}else if(deviceEvent.bits.dprFull && t8<3){
		lcd_show_string_l0((uint8_t*)"full");
	}else{
	#else
	if(hardStatus.bits.bDprSensor==0){
		lcd_show_string_l0((uint8_t*)" err");
	}else if(deviceEvent.bits.dprFull){
		lcd_show_string_l0((uint8_t*)"full");
	}else{		
	#endif
		if(t8==0){
	...
	if(t8==0){lcd_disp_unit_1st_m(true);	}
	else if(t8==1){lcd_disp_unit_1st_m3(true);	}
...
void ui_disp_menu_poly_c_adj(void)
{
	...
	/*
	switch(t8){
		case 0:buf[3]='0'+t8;break;
		case 1:buf[3]='3'+t8;break;
		case 2:buf[3]='6'+t8;break;
		default:break;
	}
	*/
...
void ui_disp_menu_wake_up_period(void)
{
	...
	if(t16==WAKE_UP_SAMPLE_FORBID){
		lcd_show_string((uint8_t*)(" slp 0ff"));
		lcd_disp_refresh(); 
	}else{
		ui_disp_adj_xfixed_static((uint8_t*)(" slp"),(int)t16,3);
	}
}
...
void ui_disp_menu_rf_send_period(void)
{
	...
	t16=*((uint16_t*)(&adjValue));
	if(t16==RF_SEND_OFF){
		lcd_show_string((uint8_t*)(" rfp   0"));
		lcd_disp_refresh(); 		
	}else{
		//ui_disp_adj_xfixed_static((uint8_t*)(" rfp"),(int)t16,3);
		ui_disp_adj_xfixed_static_3((uint8_t*)(" rfp"),(int)t16,3);
	}
}
...
void ui_disp_menu_poly_coefic_adj_mod(void)
{
	...
	//lcd_disp_logo(true);
	if(adjValue==0){
		m_mem_cpy(buf,(uint8_t*)"   n   0");
	}else if(adjValue==1){
		m_mem_cpy(buf,(uint8_t*)"   n   3");
	}else{
...
void ui_disp_menu_lcd_on_tm(void)
{
	...
	ui_disp_adj_xfixed_static_3((uint8_t*)(" lcd"),(uint8_t)adjValue,3);
}
void ui_disp_menu_modbus_id(void)
{
	lcd_clear_all();
	//ui_disp_adj_xfixed_pt_t8((uint8_t*)"  id",(int8_t)adjValue,adjLocation);
	ui_disp_adj_xfixed_static_3((uint8_t*)("  id"),(uint8_t)adjValue,3);
}
...
void ui_disp_menu(void)
{
	...
	switch(menu){
		...
		#endif
		case MENU_SHOW_PDC:				ui_disp_menu_pdc();break;
		...
		//add 
		case MENU_SET_PDC:			ui_disp_menu_set_pdc();break;
		default:break;
	}	
}
//file end
